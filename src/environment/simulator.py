...
class GroundTruthSimulator:
    ...
    def step_joint(self, actions):
        ...
        for agent_id, action in actions.items():
            if self.is_at_goal(agent_id):
                proposed_positions[agent_id] = (
        ...
        for i in range(len(agent_ids)):
            for j in range(
                ...
                if (
                    proposed_positions[a]
                    == current_positions[b]
                    and proposed_positions[b]
                    == current_positions[a]
                ):
                    edge_swap_conflicts.append(
                        (a, b)
                    )

        collision = bool(
            vertex_conflicts
            or edge_swap_conflicts
        )
